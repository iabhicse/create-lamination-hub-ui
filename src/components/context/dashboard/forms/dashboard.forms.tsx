...
import { SchemaKey, schemaMap, SchemaType } from "./dashboard.main";
...
// 🧩 Hook Factory
export const InstanceUseProfileForm = (schemaKey: SchemaKey) => {
  const currentProfile = ProfileForm.filter((item) => item.id === schemaKey);
  return useForm<SchemaType<SchemaKey>>({
    resolver: zodResolver(schemaMap[schemaKey]),
    defaultValues: initialValues(currentProfile[0]),
    mode: "onChange",
